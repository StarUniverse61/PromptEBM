...
from ibebm.dgmvae.utils import prepare_dirs_loggers, get_time
...
#------------------------------ parser -----------------------------------#
from ibebm.dgmvae.utils import str2bool, process_config
import argparse
import logging
...
from ibebm.dgmvae.models.model_bases import summary
import torch
# from ebm_models.dgmvae.dataset.corpora import PAD, EOS, EOT
...
def evaluation(model, test_feed, train_feed, evaluator, epoch, kl_weight, classifier):
    ...
    if config.forward_only:
        test_file = os.path.join(config.ebm_log_dir, config.load_sess,
                                 "{}-test-{}.txt".format(get_time(), config.gen_type))
        dump_file = os.path.join(config.ebm_log_dir, config.load_sess,
                                 "{}-z.pkl".format(get_time()))
        model_file = os.path.join(config.ebm_log_dir, config.load_sess, "model")
        sampling_file = os.path.join(config.ebm_log_dir, config.load_sess,
