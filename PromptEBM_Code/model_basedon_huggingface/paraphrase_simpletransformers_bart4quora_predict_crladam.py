...
parser.add_argument("--crladam_anneal_fun", type=str, default='sigmoid', choices=["sigmoid", "linear", 'constant'],
                    help="the type of annealing function in RecAdam. Default sigmoid")
parser.add_argument("--crladam_anneal_k", type=float, default=0.5, help="k for the annealing function in RecAdam.")
parser.add_argument("--crladam_anneal_t0", type=int, default=1000, help="t0 for the annealing function in RecAdam.")
parser.add_argument("--crladam_anneal_w", type=float, default=1.0,
                    help="Weight for the annealing function in RecAdam. Default 1.0.")
parser.add_argument("--crladam_pretrain_cof", type=float, default=5000.0,
...
from model_basedon_huggingface.model_args import Seq2SeqArgs
...
'''
#model_dir='outputs/best_model/output/output_bart/alexa_qr/checkpoint-27300-epoch-20'
#model_dir='/efs-storage/models/bart_quora_paraphrase_models_crladam/checkpoint-10260-epoch-30'
# model_dir='output/output_bart/quora_qr_lll/checkpoint-46660-epoch-20/'

model = Seq2SeqModel(
     encoder_decoder_type="bart",
     #encoder_decoder_name="/home/ec2-user/workspaces/hoverboard-workspaces/src/facebook/bart-large",
     encoder_decoder_name='output/output_bart/quora_qr_lll_1/checkpoint-23340-epoch-30',
     args=model_args,
     use_cuda=True,
     #cuda_device=args.cuda_device ##if use_cuda=False, cuda_device=-1, yet, use_cuda=True, cuda_device=-1, means we use cuda:0
)
model_dir = 'output/output_bart/quora_qr_lll_1/checkpoint-23340-epoch-30'
#model_args.best_model_dir = model_dir
