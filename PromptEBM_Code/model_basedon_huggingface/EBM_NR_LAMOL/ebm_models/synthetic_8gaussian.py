...
'''
flow, instead of ebm, put it above the generator, 
denoising, difusion model, 
flow, top to down, no need to use VAE/MCMC。non-gaussian, more concise, problem is hard to design,  
MCMC output and energy function, (may be wrong), becomes a network, graph, rather than 
output is correct, 
MCMC for inference, 
above VAE is flow, 
prior as Gaussian, after non-linear transformation, it should not be a Gaussian (through rearamerization trick).
encoder, autogressive,  normalized flow works some way,  

latent space
end to end:
latent ebm not so meaningful.
embedding with a model
latent flow is a good direction,  
1. flow, vae, gan
2. ebm, abp: au back, co-opnet
salience prediction
'''
...
def main():
    ...
    parser.add_argument('--niters', type=int, default=25000)
    ...
    parser.add_argument('--num_cls', type=int, default=8)
