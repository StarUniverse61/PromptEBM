...
import json
import random
from model_basedon_huggingface.EBM_NR_LAMOL.ebm_models.dgmvae import evaluators, utt_utils
from model_basedon_huggingface.EBM_NR_LAMOL.ebm_models.dgmvae.dataset import corpora
from model_basedon_huggingface.EBM_NR_LAMOL.ebm_models.dgmvae.dataset import data_loaders
from model_basedon_huggingface.EBM_NR_LAMOL.ebm_models.dgmvae.models.sent_models import *
from model_basedon_huggingface.EBM_NR_LAMOL.ebm_models.dgmvae.utils import get_time
from model_basedon_huggingface.EBM_NR_LAMOL.settings import TOKENIZER, SPECIAL_TOKEN_IDS, FILL_VAL, SAVE_NAME, FINAL_SAVE_NAME, TOKENS_WEIGHT, CONFIG_NAME, parser
from model_basedon_huggingface.EBM_NR_LAMOL.ebm_models.dgmvae.multi_bleu import multi_bleu_perl
#from model_basedon_huggingface.EBM_NR_LAMOL.ebm_models.dgmvae.options import get_parser
from model_basedon_huggingface.EBM_NR_LAMOL.utils_lamol import *
import datetime
...
from ibebm.dgmvae.dataset.corpora import PAD, BOS, EOS, UNK
from torch.autograd import Variable
from ibebm.dgmvae import criterions
from ibebm.dgmvae.enc2dec.decoders import DecoderRNN
from ibebm.dgmvae.enc2dec.encoders import EncoderRNN
from ibebm.dgmvae.utils import INT, FLOAT, LONG, cast_type
from ibebm.dgmvae import nn_lib
import numpy as np
from ibebm.dgmvae.models.model_bases import BaseModel
from ibebm.dgmvae.enc2dec.decoders import GEN, TEACH_FORCE
from ibebm.dgmvae.utils import Pack, kl_anneal_function, interpolate, idx2onehot
import itertools
import math

from ibebm.dgmvae.models.model_bases import summary
...
class GMVAE4Lamol(BaseModel):
    def __init__(self, corpus, config):
        super(GMVAE4Lamol, self).__init__(config)
        self.vocab = corpus.vocab
        self.rev_vocab = corpus.rev_vocab
        self.vocab_size = len(self.vocab)
        self.embed_size = config.embed_size
        self.max_utt_len = config.max_utt_len
        self.go_id = self.rev_vocab['__ans__']
        self.eos_id = self.rev_vocab['<|endoftext|>']
        self.unk_id = self.rev_vocab['__unk__']
        self.pad_id = self.rev_vocab['__pad__']
        self.gen_id = self.rev_vocab['__gen__']
        self.ans_id = self.rev_vocab['__ans__']
        ...
        self.embedding = nn.Embedding(num_embeddings=self.vocab_size if config.max_vocab_cnt<self.vocab_size else config.max_vocab_cnt,
                                      embedding_dim=self.embed_size, padding_idx=self.rev_vocab[PAD])
        self.dec_embedding = nn.Embedding(num_embeddings=self.vocab_size if config.max_vocab_cnt<self.vocab_size else config.max_vocab_cnt,
                                          embedding_dim=self.embed_size, padding_idx=self.rev_vocab[PAD])
        ...
        self.decoder = DecoderRNN(self.vocab_size, self.max_dec_len,
                                  ...
                                  output_size=config.max_vocab_cnt)
    ...
    def add_args(parser):
        ...
        parser.add_argument('--max_vocab_cnt', type=int, default=32011)
...
import argparse
import logging
# import ebm_models.dgmvae.models.sent_models as sent_models
# import ebm_models.dgmvae.models.sup_models as sup_models
# import ebm_models.dgmvae.models.dialog_models as dialog_models
...
#         data_dir = config.ebm_data_dir

#         load_path = os.path.join(config.ebm_log_dir, load_sess, "params.json")
...
# def get_more_parser(parser, model_class="sent_models"):
...
#------------------------------ engine -----------------------------------# 
import numpy as np
from model_basedon_huggingface.EBM_NR_LAMOL.ebm_models.dgmvae.models.model_bases import summary
import torch
from model_basedon_huggingface.EBM_NR_LAMOL.ebm_models.dgmvae.dataset.corpora import PAD, EOS, EOT
from model_basedon_huggingface.EBM_NR_LAMOL.ebm_models.dgmvae.enc2dec.decoders import TEACH_FORCE, GEN, DecoderRNN
from model_basedon_huggingface.EBM_NR_LAMOL.ebm_models.dgmvae.utils import get_dekenize, experiment_name, kl_anneal_function
import os
from collections import defaultdict
import logging
from model_basedon_huggingface.EBM_NR_LAMOL.ebm_models.dgmvae import utt_utils
...
def train(model, task_id, train_feed, valid_feed, test_feed, config, evaluator, gen=None):
    ...
    # save model
    torch.save(model.state_dict(), os.path.join(config.session_dir, "model_ckpt_" + str(task_id) + ".pt"))
...
#------------------------------ main -----------------------------------# 
'''
data_paths_train = ['./data/srl_to_squad-train-v2.0.json']
data_paths_valid = ['./data/srl_to_squad-dev-v2.0.json']
data_paths_test = ['./data/srl_to_squad-test-v2.0.json']
'''
def get_corpus_client(config, data_paths_train=None, data_paths_valid=None, data_paths_test=None):
    if config.ebm_data.lower() == "ptb":
        ...
        gen_token = "GEN"
        corpus_client = corpora.QACorpus(config, gen_token, SPECIAL_TOKEN_IDS, data_paths_train, data_paths_valid, data_paths_test)
    else:
...
def get_model(corpus_client, config):
    try:
        model = eval(config.ebm_model)(corpus_client, config)
        if config.forward_only:
        ...
        raise NotImplementedError("Fail to build model %s" % (config.ebm_model))
    if config.use_gpu:
        model.cuda()
    return model
...
def evaluation(model, test_feed, train_feed, evaluator, epoch, kl_weight):
    if args.forward_only:
        test_file = os.path.join(args.session_dir, "eval-{}-test-{}.txt".format(get_time(), args.gen_type))
        sampling_file = os.path.join(args.session_dir, "eval-{}-sampling.txt".format(get_time()))
    else:
        test_file = os.path.join(args.session_dir, "epoch-{:0>3d}-test-{}.txt".format(epoch, args.gen_type))
        sampling_file = os.path.join(args.session_dir, "ep-{:0>3d}-sampling.txt".format(epoch))


    if hasattr(model, "sampling_for_likelihood") and kl_weight >= args.max_kl_weight:
        ll = utt_utils.calculate_likelihood(model, test_feed, 500, args)  # must
        logger.info('log-likelihood: {:10.3f}'.format(ll))

    if kl_weight >= args.max_kl_weight:
        with open(os.path.join(sampling_file), "w") as f:
            print("Saving test to {}".format(sampling_file))
            utt_utils.exact_sampling(model, 46000, args, dest_f=f, sampling_batch_size=100)

    with open(os.path.join(test_file), "w") as f:
        print("Saving test to {}".format(test_file))
        utt_utils.generate(model, test_feed, args, evaluator, num_batch=None, dest_f=f)

    multi_bleu_perl(test_file, args.session_dir)
    model.train()
...
def main():
    set_seed(args.seed)
    task_ids = list(range(len(args.tasks)))
    ...
    if len(TOKENIZER) != TOKENS_WEIGHT.shape[0]:
        #TOKENS_WEIGHT = torch.cat((TOKENS_WEIGHT, torch.ones([1])))
        if args.use_gpu:
            TOKENS_WEIGHT = torch.cat((TOKENS_WEIGHT, torch.ones([1]).cuda()))


    if args.forward_only is False:
        ...
        for task_id in range(len(args.tasks)):
            ...
            args.max_utt_len = max_utt_lens[task_id]
            args.max_dec_len = max_dec_lens[task_id]
            train_dataset = [TASK_DICT[task_name]["train"]]
            train_extra_data = []
            train_corpus = getQA_dataset(train_dataset, "train", tasks, train_extra_data)
            max_train_batch_size = max(len(train_corpus) // args.min_n_steps, args.min_batch_size)
            # train_feed = create_dataloader(train_corpus, "train", max_train_batch_size)
            # valid_feed = getQA_dataset(train_dataset, "valid", tasks, train_extra_data)
            # test_feed = getQA_dataset(train_dataset, "test", tasks, train_extra_data)
            valid_dataset = [TASK_DICT[task_name]["eval"]]
            test_dataset = [TASK_DICT[task_name]["test"]]
            corpus_client = get_corpus_client(args, train_dataset, valid_dataset, test_dataset)
            dial_corpus = corpus_client.get_corpus()
            evaluator = evaluators.BleuEvaluator("CornellMovie")
            train_feed, valid_feed, test_feed = get_dataloader(args, dial_corpus)
            model = get_model(corpus_client, args)
            train(model, task_id, train_feed, valid_feed, test_feed, args, evaluator, gen=utt_utils.generate)
    else:
        train_extra_data = []
        ...
        test_feed = getQA_dataset(train_dataset, "test", tasks, train_extra_data)
        evaluator = evaluators.BleuEvaluator("CornellMovie")
        corpus_client = get_corpus_client(args)
        model = get_model(corpus_client, args)
        evaluation(model, test_feed, train_feed, evaluator, epoch=0, kl_weight=args.max_kl_weight)
...
if __name__ == "__main__":
    ...
    #config = get_more_parser(parser)
    try:
        model_name = args.ebm_model
        model_class = eval(model_name)
        #parser = model_class.add_args(parser)
    except Exception as e:
        raise ValueError("Wrong model=" + args.ebm_model)

    #config, _ = parser.parse_known_args()
    # print(args)

    with torch.cuda.device(args.gpu_idx):
        main()
