...
from model_basedon_huggingface.EBM_NR_LAMOL.ebm_models.dgmvae import criterions
from model_basedon_huggingface.EBM_NR_LAMOL.ebm_models.dgmvae.enc2dec.decoders import DecoderRNN
from model_basedon_huggingface.EBM_NR_LAMOL.ebm_models.dgmvae.enc2dec.encoders import EncoderRNN
from model_basedon_huggingface.EBM_NR_LAMOL.ebm_models.dgmvae.utils import INT, FLOAT, LONG, cast_type
from model_basedon_huggingface.EBM_NR_LAMOL.ebm_models.dgmvae import nn_lib
import numpy as np
from model_basedon_huggingface.EBM_NR_LAMOL.ebm_models.dgmvae.models.model_bases import BaseModel
from model_basedon_huggingface.EBM_NR_LAMOL.ebm_models.dgmvae.enc2dec.decoders import GEN, TEACH_FORCE
from model_basedon_huggingface.EBM_NR_LAMOL.ebm_models.dgmvae.utils import Pack, kl_anneal_function, interpolate, idx2onehot
import itertools
import math
import logging
...
class GMVAE4Lamol(BaseModel):
    ...
    @staticmethod
    def add_args(parser):
        from ibebm.dgmvae.utils import str2bool
        # Latent variable:
        parser.add_argument('--latent_size', type=int, default=40,
                            help="The latent size of continuous latent variable.")
    ...
    def get_optimizer(self, config):
        if config.op == 'adam':
            return torch.optim.Adam([
                ...
                {'params': [p[1] for p in self.named_parameters() if 'ebm' in p[0] and p[1].requires_grad],
                 'lr': 0.0001},
            ],
                lr=config.init_lr)
    ...
    def sample_langevin_prior_z(self, z, verbose=False):
        ...
        for i in range(args.e_l_steps):
            en = - self.ebm_prior(z)
            # en.requires_grad = True
            en_sum = en.sum()
            # en_sum.requires_grad = True
            # en.allow_unused = True
            # en_sum.allow_unused = True
            # z.allow_unused = True
            z_grad = torch.autograd.grad(en_sum, z)[0]
    ...
    def forward4lamol(self, data_feed, vae_kl_weight, gen_type, mode):
        ...
        if type(x_last) is tuple:
            x_last = \
            x_last[0].view(self.num_layer_enc, 1 + int(self.bi_enc_cell), -1, self.enc_cell_size)[-1]
            x_last = x_last.transpose(0, 1).contiguous().view(-1, self.enc_out_size)
        else:
            x_last = x_last.view(self.num_layer_enc, 1 + int(self.bi_enc_cell), -1, self.enc_cell_size)[
                -1]
            x_last = x_last.transpose(0, 1).contiguous().view(-1, self.enc_out_size)
        ...
        return dec_inputs, posterior_sample_n, sample_z, dec_init_state

    def compute_posterior(self, mode, dec_ctx, labels, dec_outs, posterior_sample_n,
                          qz_logvar, qz_mean, sample_z, batch_cnt, return_latent, dec_init_state):
    ...
    def forward(self, data_feed, mode, gen_type='greedy', sample_n=1, batch_cnt=1, return_latent=False,
                vae_kl_weight=1.):
        posterior_sample_n = self.posterior_sample_n if self.training else 1
        ...
        if mode == GEN:
            ...
            # E_p(z) (f(z))
            z_e_0 = self.sample_p_0(n=self.config.batch_size)
            prior_sample_z, prior_z_grad_norm = self.sample_langevin_prior_z(z_e_0, verbose=(
                        batch_cnt % 500 == 0) if batch_cnt else False)
            prob_neg = self.ebm_prior(prior_sample_z.detach()).mean()
    ...
    def sampling_for_likelihood(self, batch_size, data_feed, sample_num, sample_type="LL",
        ...
        output_embedding = self.embedding(out_utts)
        x_outs, x_last = self.x_encoder(output_embedding)
        # x_outs.requires_grad = True
        # x_last.requires_grad = True
        if type(x_last) is tuple:
            x_last = x_last[0].view(self.num_layer_enc, 1 + int(self.bi_enc_cell), -1, self.enc_cell_size)[-1]
            x_last = x_last.transpose(0, 1).contiguous().view(-1, self.enc_out_size)
        else:
        ...
        dec_outs, dec_last, outputs = self.decoder(sample_z.size(0),
                                                   ...
                                                   latent_variable=sample_z if self.concat_decoder_input else None)
        # dec_outs.requires_grad = True
        # dec_last.requires_grad = True
    ...
    def generate_with_past(self, batch_size, past):
...
class DiVAE(BaseModel):
    ...
    def forward(self, data_feed, mode, past=None, gen_type='greedy', sample_n=1, return_latent=False):
        posterior_sample_n = self.posterior_sample_n if self.training else 1
    ...
    # '''
    # different from original ebm_models, sampling now generate one token each time rather than generatie all sentences.
    # Therefore, we need to keep past and also, input should be the last token stored by past.
    # Right now, ebm_models use lstm, the autoregressive algorithm just consider hidden state and previous state.
    # '''
    # def sampling_dc(self, batch_size, past):
    #     sample_y = torch.randint(0, self.config.k, [batch_size, self.config.mult_k], dtype=torch.long).cuda()
    #     cs = self.torch2var(idx2onehot(sample_y.view(-1), self.config.k)).view(-1, self.config.mult_k * self.config.k)
