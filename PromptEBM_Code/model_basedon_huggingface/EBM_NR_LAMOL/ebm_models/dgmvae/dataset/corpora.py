...
import numpy as np
import json
from model_basedon_huggingface.EBM_NR_LAMOL.ebm_models.dgmvae.utils import get_tokenize, get_chat_tokenize, missingdict, Pack
from model_basedon_huggingface.EBM_NR_LAMOL.settings import FILL_VAL
...
class QACorpus(object):
    ...
    def __init__(self, config, gen_token, SPECIAL_TOKEN_IDS, data_paths_train, data_paths_valid, data_paths_test):
        self.config = config
    ...
    def _build_vocab(self):
        # self.rev_vocab = TOKENIZER.encoder
        self.rev_vocab = TOKENIZER.vocab
        self.unk_id = TOKENIZER.added_tokens_encoder['__unk__']
        self.pad_id = TOKENIZER.added_tokens_encoder['__pad__']
        # self.sys_id = TOKENIZER.added_tokens_encoder[SYS]
        # self.usr_id = TOKENIZER.added_tokens_encoder[USR]
        self.gen_id = TOKENIZER.added_tokens_encoder['__gen__']
        self.ans_id = TOKENIZER.added_tokens_encoder['__ans__']

        # self.vocab = ['__pad__', '__unk__', SYS, USR, '__gen__', '__ans__'] + [t for t in TOKENIZER.encoder]
        self.vocab = ['__pad__', '__unk__', SYS, USR, '__gen__', '__ans__'] + [t for t in TOKENIZER.vocab]
    ...
    def _to_id_corpus(self, data):
        ...
        for line in data:
            if line is None:
                print("Warning: Encountered None in data.")
                continue  # Skip this iteration if line is None
            if len(line) < 7:  # Check if the line has at least 7 elements to safely access line[6]
                print("Warning: Data line too short, skipping:", line)
                continue
...
class PTBCorpus(object):
    ...
    def _to_id_corpus(self, data):
        ...
        for line in data:
            ...
            id_turn = Pack(utt=self._sent2id(line.utt),
                           speaker=line.speaker,
                           meta=line.get('meta', {}))  # Provide a default value for meta if it's not available
            results.append(id_turn)
        return results
