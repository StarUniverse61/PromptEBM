...
from model_basedon_huggingface.EBM_NR_LAMOL.settings import args, TASK_DICT, SPECIAL_TOKENS, SPECIAL_TOKEN_IDS, FILL_VAL
from model_basedon_huggingface.EBM_NR_LAMOL.settings import TOKENIZER, LEN_FACTOR, DATA_ATTRS, MEMORY_FACTOR, MODEL_CONFIG, MODEL_CLASS
...
from model_basedon_huggingface.EBM_NR_LAMOL.ebm_models.dgmvae import main as engine
from model_basedon_huggingface.EBM_NR_LAMOL.ebm_models.dgmvae.enc2dec.decoders import GEN, DecoderRNN, TEACH_FORCE
...
def get_losses(parallel_model, cqa, Y, gen_X, gen_Y, loss_fct):
    if "lll" in args.seq_train_type:
        # logger.info(f'type of cqa = {type(cqa)} and length of cqa = {len(cqa[0])}')
        # for i, item in enumerate(cqa):
        #     for j, item_j in enumerate(item):
        #         logger.info(f'j = {j} and item.shape of cqa = {item_j.shape}')
        #         logger.info(item_j)
        qa_logits = parallel_model(cqa)
        #logger.info(f'qa_logits shape is {qa_logits[0].shape}')
        lm_logits = parallel_model(gen_X)
        # logger.info(f'type of gen_X = {type(gen_X)} and length of gen_X = {len(gen_X[0])}')
        # for item in gen_X:
        #     for j, item_j in enumerate(item):
        #         logger.info(f'j = {j} and item.shape of gen_X = {item_j.shape}')
        #         logger.info(item_j)
        # logger.info(f'type of gen_Y = {type(gen_Y)} and length of gen_Y = {len(gen_Y[0])}')
        # for item in gen_Y:
        #     for j, item_j in enumerate(item):
        #         logger.info(f'j = {j} and item.shape of gen_Y = {item_j.shape}')
        #         logger.info(item_j)
        #
        # logger.info(f'lm_logits shape is {lm_logits[0].shape}')
        qa_loss = loss_fct([torch.transpose(l, 1, 2) for l in qa_logits], Y)
        # logger.info(f'type of qa_loss is {type(qa_loss)}')
        # logger.info(f'qa_loss is {qa_loss}')
        lm_loss = loss_fct([torch.transpose(l, 1, 2) for l in lm_logits], gen_Y)
        # logger.info(f'type of qa_loss is {type(lm_loss)}')
        # logger.info(f'qa_loss is {lm_loss}')
...
class QADataset(Dataset):
    def __init__(self, data_paths, data_type, gen_token, extra_data=[]):
        ...
        data = []   ###['./data/sst_to_squad-train-v2.0.json', './data/srl_to_squad-train-v2.0.json', './data/woz.en_to_squad-train-v2.0.json']
    ...
    def parse_example(self, gen_token, context, question, answer, idx):
        ...
        if Y_example is None:
            print("Warning: Y_example is None for index:", idx)
            Y_example = [FILL_VAL] * len(cqa_example)  # Adjust as needed
        else:
...
class TrainStep:
    ...
    def __call__(self, loss, scheduler_steps):
        if not args.fp32:
            ...
            #logger.info(loss)
            loss.backward()
            #logger.info(f'pass loss.backward()')
        if not args.fp32:
...
def sample_ebm_sequence(ebm_model, data_feed, need_process, qa_results, all_pasts, max_tot_lens, kl_weight=1., num_batch=1, dest_f=None, gen_size=1283):
    ...
    while len(need_process) > 0:
        ...
        while not stop:
            ...
            dec_input, posterior_sample_n, sample_z, dec_init_state = utt_utils.generate4lamol(ebm_model, batch,
                                                                                               "train", evaluator)
            while True:
                try:
                    cur_id = next(it)
            ...
            '''
             ...
             beam_size=4, latent_variable=None
            '''
            all_outputs, dec_last, dec_ctx = ebm_model.decoder.forward4lamol(gen_size,
                                                                             input_ids.repeat(posterior_sample_n, 1),
                                                                             dec_init_state,
                                                                             # mode=mode,
                                                                             gen_type=gen_type,
                                                                             beam_size=ebm_model.beam_size,
...
#                 remove_id(idx, need_process, all_pasts)
    ...
    #             pred_str, attn = engine.get_sent(ebm_model, de_tknize, pred_labels,
    #                                              b_id, attn=pred_attns)
    #             true_str, _ = engine.get_sent(ebm_model, de_tknize, true_labels, b_id)
    ...
    # ebm_model.compute_posterior()
    # multi_bleu_perl(test_file, args.session_dir)
    # ebm_model.train()
    # return evaluator.get_report(include_error=dest_f is not None, get_value=True)

'''
The logic of sample_sequence in LAMOL's util_lamol.py:
arguments: model, need_process, qa_results, all_pasts, max_tot_lens
model: the one to generate sample, 
need_process: an orderdict with length as the total number of samples
qa_results: a list with length of need_process and each one will be filled values for generated sample sequence ids.
all_pasts: after each inner loop, generated tokens for each sample will be added to it used to help generate next tokens
max_tot_lens: the longest sentence being generated
'''
