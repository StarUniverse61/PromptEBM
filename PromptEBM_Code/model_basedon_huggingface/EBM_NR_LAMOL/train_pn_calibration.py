...
from settings import TASK_DICT, init_logging, MODEL_CONFIG, MODEL_CLASS, SPECIAL_TOKENS, CONFIG_CLASS
from settings import TOKENIZER, SPECIAL_TOKEN_IDS, FILL_VAL, SAVE_NAME, FINAL_SAVE_NAME, TOKENS_WEIGHT, CONFIG_NAME
from utils_lamol import *
...
from model_basedon_huggingface.RecAdam import RecAdam
#from model_basedon_huggingface.adam import Adam
import random
...
from ebm_models.dgmvae.dataset.corpora import PAD, BOS, EOS, UNK
from torch.autograd import Variable
from ebm_models.dgmvae import criterions
from ebm_models.dgmvae.enc2dec.decoders import DecoderRNN
from ebm_models.dgmvae.enc2dec.encoders import EncoderRNN
from ebm_models.dgmvae.utils import INT, FLOAT, LONG, cast_type
from ebm_models.dgmvae import nn_lib
import numpy as np
from ebm_models.dgmvae.models.model_bases import BaseModel
from ebm_models.dgmvae.enc2dec.decoders import GEN, TEACH_FORCE
from ebm_models.dgmvae.utils import Pack, kl_anneal_function, interpolate, idx2onehot
import itertools
import math
...
def get_model(corpus_client):
    try:
        ebm_model = eval(args.ebm_model)(corpus_client, args)
        if args.forward_only:
            ebm_model.load_state_dict(torch.load(args.ebm_model_file,map_location='cpu'))
        else:
...
def get_corpus_client(data_paths_train=None, data_paths_valid=None, data_paths_test=None):
    if "ptb" in args.ebm_data.lower():
        ...
        gen_token = "GEN"
        corpus_client = corpora.QACorpus(args, gen_token, SPECIAL_TOKEN_IDS, data_paths_train, data_paths_valid, data_paths_test)

    # elif "QA_dataset" in args.ebm_data.lower():
    #     corpus_client = QADataset
    #     corpus_client = QADataset(train_dataset, "train", SPECIAL_TOKEN_IDS[tasks[0]], train_extra_data)
...
def train(task_ids, model):
    ...
    if "lll" in args.seq_train_type and task_ids[0] > 0 and not args.skip_tasks:
        ...
        train_extra_data = []
    elif 'pn_calibration' in args.seq_train_type and task_ids[0] > 0 and not args.skip_tasks:
        prev_task = args.tasks[task_ids[0] - 1]
        with torch.no_grad():
            create_extra_data(tasks[0], prev_task, model, train_extra_data)
    elif "pn_calib_ebm" in args.seq_train_type and task_ids[0] > 0 and not args.skip_tasks:
        set_seed(args.seed)
        ...
        corpus_client = get_corpus_client(train_dataset, valid_dataset, test_dataset)
        ebm_model = get_model(corpus_client)
        dial_corpus = corpus_client.get_corpus()

        train_feed, valid_feed, test_feed = get_ebm_dataloader(args, dial_corpus)
        prev_task = args.tasks[task_ids[0]-1]
        create_extra_data_ebm(tasks[0], train_feed, prev_task, model, ebm_model, train_extra_data)
        #with torch.no_grad():
    ...
    if not model:
        ...
        MODEL_CLASS.pretrained_model_archive_map['encoder_norm_self'] = args.encoder_norm_self
        MODEL_CLASS.pretrained_model_archive_map['decoder_norm_self'] = args.decoder_norm_self
        MODEL_CLASS.pretrained_model_archive_map['encoder_norm_ff'] = args.encoder_norm_ff
        MODEL_CLASS.pretrained_model_archive_map['decoder_norm_ff'] = args.decoder_norm_ff
        MODEL_CLASS.pretrained_model_archive_map['warmup_updates'] = args.warmup_updates
        ...
        # pretrained_model = MODEL_CLASS(model_config).cuda().eval()
        ...
        if tasks[0] in args.skip_tasks:
            if len(args.skip_tasks) == 1:
                ...
                if args.seq_train_type in REG_TYPE_KEYS:
                    ...
                    train_qadata = getQA_dataset(train_dataset, "train", tasks, train_extra_data)
    ...
    elif args.seq_train_type == 'pn_calibration': #or args.seq_train_type == 'lll':
        ...
        optimizer_grouped_parameters = [
            {
                "params": [p for n, p in model.named_parameters() if
                           not any(nd in n for nd in no_decay) and args.model_name in n],
                "weight_decay": args.weight_decay,
                "anneal_w": args.recadam_anneal_w,
                "pretrain_params": [p_p for p_n, p_p in pretrained_model.named_parameters() if
                                    not any(nd in p_n for nd in no_decay) and args.model_name in p_n]
            },
            {
                "params": [p for n, p in model.named_parameters() if
                           not any(nd in n for nd in no_decay) and args.model_name not in n],
                "weight_decay": args.weight_decay,
                "anneal_w": 0.0,
                "pretrain_params": [p_p for p_n, p_p in pretrained_model.named_parameters() if
                                    not any(nd in p_n for nd in no_decay) and args.model_name not in p_n]
            },
            {
                "params": [p for n, p in model.named_parameters() if
                           any(nd in n for nd in no_decay) and args.model_name in n],
                "weight_decay": 0.0,
                "anneal_w": args.recadam_anneal_w,
                "pretrain_params": [p_p for p_n, p_p in pretrained_model.named_parameters() if
                                    any(nd in p_n for nd in no_decay) and args.model_name in p_n]
            },
            {
                "params": [p for n, p in model.named_parameters() if
                           any(nd in n for nd in no_decay) and args.model_name not in n],
                "weight_decay": 0.0,
                "anneal_w": 0.0,
                "pretrain_params": [p_p for p_n, p_p in pretrained_model.named_parameters() if
                                    any(nd in p_n for nd in no_decay) and args.model_name not in p_n]
            }
        ]
    ...
    elif args.seq_train_type == 'lll' or args.seq_train_type == 'pn_calib_ebm':
    ...
    for ep in range(n_train_epochs):
        ...
        for n_steps, (_, _, cqa, _, Y, gen_X, gen_Y) in enumerate(train_dataloader):
            ...
            #logger.info(f'type of n_inputs = {type(n_inputs)}')
            #logger.info(n_inputs)
