...
from CRLAdam import CRLAdam as CrlAdam, compute_omega_grads_norm, CRLAdam_omega_update
import numpy as np
import pandas as pd
import torch
...
class Seq2SeqModel:
    def __init__(
        ...
        **kwargs,
        ...
        #dist.init_process_group(backend="nccl", init_method='tcp://10.0.38.33:23456', world_size=4, rank=args.local_rank)
        ...
        if encoder_decoder_type in ["bart", "mbart", "marian"]:
            ...
            if encoder_decoder_type in ["bart", "mbart"]:
                self.pretrained_model = model_class.from_pretrained(encoder_decoder_name)
                if encoder_decoder_type in ["bart", "mbart"]:
                ...
                if args.start_from_pretrain:
                    self.new_model = model_class.from_pretrained(encoder_decoder_name)
                else:
                    #self.new_model = model_class(config=config)
                    self.new_model = model_class(config=config)
            ...
            self.config = self.new_model.config
        else:
            if encoder_decoder_name:
                ...
                self.pretrained_model = EncoderDecoderModel.from_encoder_decoder_pretrained(
                    os.path.join(encoder_decoder_name, "encoder"), os.path.join(encoder_decoder_name, "decoder"),
                ...
                if args.start_from_pretrain:
                    self.new_model = EncoderDecoderModel.from_encoder_decoder_pretrained(
                        os.path.join(encoder_decoder_name, "encoder"), os.path.join(encoder_decoder_name, "decoder"),
                        from_tf=bool(".ckpt" in args.model_name_or_path),
                        config=config,
                        cache_dir=args.cache_dir if args.cache_dir else None,
                ...
                # self.pretrained_model = model_class.from_pretrained(
                ...
                # )
                # if args.start_from_pretrain:
                #     self.new_model = model_class.from_pretrained(
                #         args.model_name_or_path,
                #         from_tf=bool(".ckpt" in args.model_name_or_path),
                #         config=config,
                #         cache_dir=args.cache_dir if args.cache_dir else None,
            ...
            self.encoder_config = self.new_model.config.encoder
            self.decoder_config = self.new_model.config.decoder
    ...
    def train_model(
        ...
        new_args['crladam_anneal_fun']=args.recadam_anneal_fun
        new_args['crladam_anneal_k']=args.recadam_anneal_k
        new_args['crladam_anneal_t0']=args.recadam_anneal_t0
        new_args['crladam_pretrain_cof']=args.recadam_pretrain_cof
        new_args['crladam_anneal_w']=args.recadam_anneal_w
        new_args['start_from_pretrain'] = args.start_from_pretrain

        if args:
            self.args.update_from_dict(new_args)
        ...
        self.save_model(self.args.output_dir, model=self.new_model)
    ...
    def train(
        ...
        #     data = self._get_inputs_dict(data)
        #     print('debug')
        ...
        for group in self.args.custom_parameter_groups:
            ...
            param_group = {**group}
            param_group["params"] = [p for n, p in self.new_model.named_parameters() if n in params]
        ...
        for group in self.args.custom_layer_parameters:
            ...
            params_d = []
            params_nd = []
            for n, p in self.new_model.named_parameters():
        ...
        if args.optimizer == "AdamW":
        ...
        elif args.optimizer == 'CrlAdam':
            # Prepare for the grouped parameters for CrlAdam optimizer.
            # Since the classifier layer is not pretrained, it is not penalized during optimization.
            optimizer_grouped_parameters = [
                {
                    "params": [p for n, p in self.new_model.named_parameters() if
                               not any(nd in n for nd in no_decay) and args.model_type in n],
                    "weight_decay": args.weight_decay,
                    "anneal_w": args.crladam_anneal_w,
            ...
            optimizer = CrlAdam(optimizer_grouped_parameters, reg_lambda=args.reg_lambda, lr=args.learning_rate, eps=args.adam_epsilon,
                                anneal_fun=args.crladam_anneal_fun, anneal_k=args.crladam_anneal_k,
                                anneal_t0=args.crladam_anneal_t0, pretrain_cof=args.crladam_pretrain_cof)
        else:
            raise ValueError(
        ...
        if args.n_gpu > 1:
            self.new_model = torch.nn.DataParallel(self.new_model)
        ...
        for current_epoch in train_iterator:
            ...
            for step, batch in enumerate(batch_iterator):
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0:
                    ...
                    if args.optimizer == "AdamW":
                        torch.nn.utils.clip_grad_norm_(self.new_model.parameters(), args.max_grad_norm)

                    if args.fp16:
                        scaler.step(optimizer, reg_params)
                        scaler.update()
                    else:
                        optimizer.step(reg_params=reg_params)
                    ...
                    if args.save_steps > 0 and global_step % args.save_steps == 0:
                        ...
                        self.save_model(output_dir_current, optimizer, scheduler, model=self.new_model)
                    ...
                    if args.evaluate_during_training and (
                        ...
                        if args.save_eval_checkpoints:
                            self.save_model(output_dir_current, optimizer, scheduler, model=self.new_model, results=results)
                        ...
                        if not best_eval_metric:
                            ...
                            if args.save_best_model:
                                self.save_model(
                                    args.best_model_dir, optimizer, scheduler, model=self.new_model, results=results
                                )
            ...
            epoch_number += 1
            # update omega after each epoch
            optimizer_ft = CRLAdam_omega_update(reg_params)
            self.new_model, reg_params = compute_omega_grads_norm(args, self.new_model, reg_params, train_dataloader,
                                                             optimizer_ft)
            self.new_model.eval()
            ...
            if args.evaluate_during_training and args.evaluate_each_epoch:
                ...
                if not best_eval_metric:
                    best_eval_metric = results[args.early_stopping_metric]
                    if args.save_best_model:
                        self.save_model(args.best_model_dir, optimizer, scheduler, model=self.new_model, results=results)
                if best_eval_metric and args.early_stopping_metric_minimize:
                    if results[args.early_stopping_metric] - best_eval_metric < args.early_stopping_delta:
                        best_eval_metric = results[args.early_stopping_metric]
                        if args.save_best_model:
    ...
    def predict(self, to_predict):
        ...
        for batch in [
            ...
            if self.args.model_type in ["bart", "marian"]:
                outputs = self.new_model.generate(
                    input_ids=input_ids,
                ...
                outputs = self.new_model.generate(
                    input_ids=input_ids,
                    decoder_start_token_id=self.new_model.config.decoder.pad_token_id,
                    num_beams=self.args.num_beams,
                    max_length=self.args.max_length,
                    length_penalty=self.args.length_penalty,
                    early_stopping=self.args.early_stopping,
    ...
    def save_model(self, output_dir=None, optimizer=None, scheduler=None, model=None, results=None):
        ...
        if model and not self.args.no_save:
            ...
            if self.args.model_type in ["bart", "mbart", "marian"]:
                ...
                model_to_save = (
                    self.new_model.encoder.module if hasattr(self.new_model.encoder, "module") else self.new_model.encoder
                )
                model_to_save.save_pretrained(os.path.join(output_dir, "encoder"))

                model_to_save = (
                    self.new_model.decoder.module if hasattr(self.new_model.decoder, "module") else self.new_model.decoder
                )
    ...
    def get_named_parameters(self):
        return [n for n, p in self.new_model.named_parameters()]

'''
model_dict = torch.load('/efs-storage/models/bart_check_06-07/checkpoint_best.pt')
fp = open('/efs-storage/models/bart_check_06-07/model_parameter.bin', 'wb')
weight_count = 0
num=1
