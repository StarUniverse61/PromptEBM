...
from model_basedon_huggingface.norm_select import NormSelect
...
ACT_FNS = {"relu": nn.ReLU, "swish": swish, "gelu": nn.GELU}
...
class MLP(nn.Module):
    def __init__(self, n_state, config):  # in MLP: n_state=3072 (4 * n_embd)
        ...
        config.afn = "relu"
        self.act = ACT_FNS[config.afn](inplace=False)
...
class Block(nn.Module):
    def __init__(self, n_ctx, config, scale=False):
        ...
        self.ln_1 = NormSelect(config.pretrained_config_archive_map['encoder_norm_self'], nx, config.num_attention_heads,
                                               config.pretrained_config_archive_map['warmup_updates'])
        self.mlp = MLP(4 * nx, config)
        #self.ln_2 = LayerNorm(nx, eps=config.layer_norm_epsilon)
        self.ln_2 = NormSelect(config.pretrained_config_archive_map['encoder_norm_ff'], nx, config.num_attention_heads,
                                               config.pretrained_config_archive_map['warmup_updates'])
...
class OpenAIGPTModel(OpenAIGPTPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        config.pretrained_config_archive_map['encoder_norm_self']=self.pretrained_model_archive_map['encoder_norm_self']
        config.pretrained_config_archive_map['decoder_norm_self'] = self.pretrained_model_archive_map[
            'decoder_norm_self']
        config.pretrained_config_archive_map['encoder_norm_ff'] = self.pretrained_model_archive_map[
            'encoder_norm_ff']
        config.pretrained_config_archive_map['decoder_norm_ff'] = self.pretrained_model_archive_map[
            'decoder_norm_ff']
        config.pretrained_config_archive_map['warmup_updates'] = self.pretrained_model_archive_map[
            'warmup_updates']
