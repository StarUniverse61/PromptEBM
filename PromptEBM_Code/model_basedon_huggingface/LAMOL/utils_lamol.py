...
def get_losses(parallel_model, cqa, Y, gen_X, gen_Y, loss_fct):
    if "lll" in args.seq_train_type:
        #print(cqa)
        #logger.info(f'the shape of cqa = {len(cqa)}')
        #logger.info(cqa)
        #cqa[0][0].requires_grad
        qa_logits = parallel_model(cqa)
        #print(qa_logits)
        lm_logits = parallel_model(gen_X)
        #print(lm_logits)
        qa_loss = loss_fct([torch.transpose(l, 1, 2) for l in qa_logits], Y)
        #qa_loss = loss_fct([torch.transpose(l, 1, 2) for l in lm_logits], gen_Y)
        lm_loss = loss_fct([torch.transpose(l, 1, 2) for l in lm_logits], gen_Y)
        #lm_loss = loss_fct([torch.transpose(l, 1, 2) for l in lm_logits], Y)
...
'''
QADataset(train_dataset, "train", SPECIAL_TOKEN_IDS[tasks[0]], train_extra_data)
