...
from modeling_bart import BartConfig, BartForConditionalGeneration, BartEncoder, BartDecoder
...
class Seq2SeqModel:
    def __init__(
        ...
        config=None,
        args=None,
        prev_model=None, ##for lifelong learning, we start a new model with prev_model
        use_cuda=True,
        cuda_device=-1,
        **kwargs,
    ):
        ...
        ##python -m torch.distributed.launch run_para_trin.py
        ...
        config_class.attribute_map['encoder_norm_self'] = args.encoder_norm_self
        config_class.attribute_map['decoder_norm_self'] = args.decoder_norm_self
        config_class.attribute_map['encoder_norm_ff'] = args.encoder_norm_ff
        config_class.attribute_map['decoder_norm_ff'] = args.decoder_norm_ff
        config_class.attribute_map['warmup_updates'] = args.warmup_updates
        print(f"in seq2seq_model.py where encoder_norm_self={config_class.attribute_map['encoder_norm_self']} "
                f"decoder_norm_self={config_class.attribute_map['decoder_norm_self']} "
                f"encoder_norm_ff={config_class.attribute_map['encoder_norm_ff']} "
              f"decoder_norm_ff={config_class.attribute_map['decoder_norm_ff']} ")
        if encoder_decoder_type in ["bart", "mbart", "marian"]:
            ...
            if encoder_decoder_type in ["bart", "mbart"]:
                ...
                if args.start_from_pretrain:
                    #self.new_model = model_class.from_pretrained(prev_model)
    ...
    def train(
        ...
        print(f'train_batch_size in train of seq2seq_model.py is {args.train_batch_size}')
        if args.max_steps > 0:
        ...
        print(f'args.optimizer in seq2seq_model.py ={args.optimizer}')
    ...
    def eval_model(self, eval_data, output_dir=None, verbose=True, silent=False, **kwargs):
        ...
        if not output_dir:
            output_dir = self.args.output_dir
        self.device = 'cpu' ##looks that using gpu will lead to OOM
        print(f'self.device={self.device} before eval_model is done')
        self._move_model_to_device()
        ...
        print(f'self.device={self.device} after eval_model is done')
        self._move_model_to_device()
        return self.results
    ...
    def predict(self, to_predict):
        ...
        """  # noqa: ignore flake8"
        #self.device = 'cpu' ##looks that gpu is easy to be OOM
        self._move_model_to_device()

        all_outputs = []
        # Batching
        print(f'total length of predict = {len(to_predict)}')
        count = 0
        for batch in [
            ...
            print(f'count = {count} and shape of batch={len(batch)} in predict of seq2seq_model')
            print(f'max_seq_length = {self.args.max_seq_length} in predict of seq2seq_model')
    ...
    def _move_model_to_device(self):
        print(f'self.device in _move_model_to_device {self.device}')
