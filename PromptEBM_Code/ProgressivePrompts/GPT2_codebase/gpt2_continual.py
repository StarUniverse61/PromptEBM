...
class GPT2ContinualLearner:
    def __init__(self,
        ...
        if freeze_weights:
            ...
            self.do_freeze_weights(self.model, except_condition=freeze_except)
    ...
    def init_new_prompt(self, prompt_len):
        ...
        for i in range(prompt_len):
            with torch.no_grad():
                ...
                w = deepcopy(model.transformer.wte.weight[j].detach().cpu().numpy())
    ...
    def progress_previous_prompts(self, model, task=None):
        if self.early_stopping: # use best val acc prompt & MLP
            ...
            if task!=None and self.prefix_MLPs!=None:
                new_prompt = self.prefix_MLPs[task](model.prompt)
            else:
                new_prompt = model.prompt
    ...
    def restore_best_model(self, model):
        if self.prefix_len>0:
            model.prompt = nn.Parameter(torch.tensor(self.best_prompt,
                                                          requires_grad=True))
    ...
    def train_step_lester(self,
        ...
        batch = {k: batch[k][0].to(self.device) for k in range(2)}
        labels = batch[1]
        # lm_labels[lm_labels[:, :] == tokenizer.pad_token_id] = -100

        inputs_embeds = model.get_input_embeddings()(batch[0])
        self.seq_len = len(batch[0][0])
        ...
        loss_fct = CrossEntropyLoss(ignore_index=FILL_VAL, weight=TOKENS_WEIGHT)
        outputs_logits = model(inputs_embeds=inputs_embeds).logits
        loss = loss_fct(torch.transpose(outputs_logits, 1, 2), labels)
    ...
    def do_freeze_weights(self, model, except_condition='shared'):
