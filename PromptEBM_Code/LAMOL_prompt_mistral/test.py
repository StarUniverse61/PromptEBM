...
def test_one_to_one(task_load, task_eval, model, score_dict):
    ...
    all_pasts = [[0 for _ in range(n_examples)] for __ in range(MODEL_CONFIG.num_hidden_layers)]
    ...
    for i in range(len(test_qadata)):
        ...
        qa_results[i] = qa_results[i][~np.isin(qa_results[i], [50261, 50262, 50263, 50264, 50265, 50266, 50267])]
...
def test_one_to_many(task_load):
    ...
    for ep in range(args.n_train_epochs[task_load]):
        ...
        model = MODEL_CLASS.from_pretrained(args.model_name, config=model_config, quantization_config=bnb_config,
                                            device_map="auto").eval()
        # Retrieve the existing weights
        existing_embed_tokens_weight = model.get_input_embeddings().weight
        existing_lm_head_weight = model.lm_head.weight

        # Move tensors to GPU
        existing_embed_tokens_weight = existing_embed_tokens_weight.to("cuda")
        existing_lm_head_weight = existing_lm_head_weight.to("cuda")

        # Create new tensors with the new vocab size
        new_embed_tokens_weight = torch.nn.Parameter(
            torch.zeros((model_config.vocab_size, existing_embed_tokens_weight.shape[1]), device="cuda"),
            requires_grad=True)
        new_lm_head_weight = torch.nn.Parameter(
            torch.zeros((model_config.vocab_size, existing_lm_head_weight.shape[1]), device="cuda"), requires_grad=True)

        # Copy the existing weights to the new tensors
        new_embed_tokens_weight[:existing_embed_tokens_weight.shape[0]].data.copy_(existing_embed_tokens_weight.data)
        new_lm_head_weight[:existing_lm_head_weight.shape[0]].data.copy_(existing_lm_head_weight.data)

        # Update the model with the new weights and vocab size
        model.get_input_embeddings().weight = new_embed_tokens_weight
        model.lm_head.weight = new_lm_head_weight
        model.config.vocab_size = model_config.vocab_size
