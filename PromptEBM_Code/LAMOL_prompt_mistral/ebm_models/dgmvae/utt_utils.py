...
from LAMOL_prompt_mistral.ebm_models.dgmvae import main as engine
from LAMOL_prompt_mistral.ebm_models.dgmvae.enc2dec.decoders import GEN, DecoderRNN, TEACH_FORCE
...
from LAMOL_prompt_mistral.ebm_models.dgmvae import utils
...
'''
the following function is adapted from the forward function of ebm_model
'''
# def generate4lamol(ebm_model, data_feed, mode,gen_type='greedy'):
#        #, decoder_outputs, # a list of logprob
#         # sequence_symbols, # a list word ids
#         # back_pointers, # a list of parent beam ID
#         # lengths,  sample_n=1,
#         # batch_cnt=1, return_latent=False, vae_kl_weight=1.):
#     # decode
#     ##self, data_feed, vae_kl_weight, gen_type, mode
#     # dec_outs, dec_last, dec_ctx = ebm_model.forward4lamol(batch_size * posterior_sample_n,
...
#                                                     mode=mode, gen_type=gen_type)
#     return dec_outs, posterior_sample_n, sample_z, dec_init_state
...
'''
the following funtion aims at being used by LAMOL for synthetic generation.
'''
def exact_sampling_with_past(model, max_sampling_num, config, dest_f, past=None, sampling_batch_size=30):
    model.eval()
    # de_tknize = utils.get_dekenize()
    ...
    while (done_epoch * sampling_batch_size < max_sampling_num):
        ...
        outputs = model.sampling_with_past(batch_size, past)
        pred_labels = [t.cpu().data.numpy() for t in outputs[DecoderRNN.KEY_SEQUENCE]]
        pred_labels = np.array(pred_labels, dtype=int).squeeze(-1).swapaxes(0, 1)
        pred_attns = None
        done_epoch+=1
...
def exact_sampling(model, max_sampling_num, config, dest_f=None, sampling_batch_size=1282):
    model.eval()
    # de_tknize = utils.get_dekenize()
    de_tknize = lambda w_list: " ".join(w_list)
...
def draw_pics(model, data_feed, config, epoch, num_batch=10, add_text = False, shuffle=False):
    ...
    # draw pics:
    if not os.path.exists(os.path.join(config.ebm_fig_dir, config.time_stamp)):
        os.makedirs(os.path.join(config.ebm_fig_dir, config.time_stamp))
    ...
    for i in range(config.mult_k):
        ...
        fout_test_clustering = open(os.path.join(config.ebm_fig_dir, config.time_stamp, "tsne-S%i-B%i.txt" % (i, epoch)), "w")

        if config.tsne:
            # X = all_zs
            Means = model.gaussian_mus.data.cpu().numpy()[i, :, :]
            X_tsne, Means_tsne = utils.tsne(X, Means, y, os.path.join(config.ebm_fig_dir, config.time_stamp, "tsne-S%i-B%i.png" % (i, epoch)),
                       text_label=all_text if add_text else None)
            # Dumping points aftering TSNE
            json.dump({'text': all_text, 'x': X_tsne.tolist(), 'y': y.tolist(), 'Means': Means_tsne.tolist()}, open(os.path.join(config.ebm_fig_dir, config.time_stamp, "tsne-S%i-B%i.json" % (i, epoch)), "w"))
            ...
            # fig = g.get_figure()
            # fig.savefig(os.path.join(config.ebm_fig_dir, config.time_stamp, "S%i-B%i.png" % (i, epoch)), format='pdf', bbox_inches='tight')
            g.savefig(os.path.join(config.ebm_fig_dir, config.time_stamp, "S%i-B%i.png" % (i, epoch)),
                      )  # bbox_inches='tight'
            # g.savefig(os.path.join(config.ebm_fig_dir, config.time_stamp, "S%i-B%i.pdf" % (i, epoch)), format='pdf', ) # bbox_inches='tight'
            json.dump({'x': X.tolist(), 'y': y.tolist(), 'means': model.gaussian_mus.data.cpu().numpy()[i, :, :].tolist(),
                      ...
                      open(os.path.join(config.ebm_fig_dir, config.time_stamp, "tsne-S%i-B%i.json" % (i, epoch)), "w"))
    ...
    fout_test_clustering = open(os.path.join(config.ebm_fig_dir, config.time_stamp, "tsne-A-B%i.txt" % (epoch)), "w")

    if config.tsne:
        # X = all_zs
        Means = np.concatenate(model.gaussian_mus.data.cpu().numpy(), axis=1)
        X_tsne, Means_tsne = utils.tsne(X, Means, y, os.path.join(config.ebm_fig_dir, config.time_stamp,
                                                                  "tsne-A-B%i.png" % (epoch)),
                                        text_label=all_text if add_text else None)
        # Dumping points aftering TSNE
        json.dump({'text': all_text, 'x': X_tsne.tolist(), 'y': y.tolist(), 'Means': Means_tsne.tolist()},
                  open(os.path.join(config.ebm_fig_dir, config.time_stamp, "tsne-A-B%i.json" % (epoch)), "w"))
