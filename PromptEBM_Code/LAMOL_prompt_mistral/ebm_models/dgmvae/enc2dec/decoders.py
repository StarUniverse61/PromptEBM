...
from LAMOL_prompt_mistral.ebm_models.dgmvae.enc2dec.base_modules import BaseRNN
from LAMOL_prompt_mistral.ebm_models.dgmvae.utils import FLOAT, LONG, cast_type
...
class DecoderRNN(BaseRNN):
    ...
    def forward_step(self, input_var, hidden, encoder_outputs, latent_variable=None):
        ...
        #temp = torch.ones([1283, 1], dtype=torch.LongTensor)
        # temp = torch.ones([1283, 1], dtype=torch.int32).cuda()
        # embedded = self.embedding(temp)
        #temp = torch.ones([1283, 1], dtype=torch.LongTensor)
        # temp = torch.ones([1283, 1], dtype=torch.int32).cuda()
        # temp = temp*50259
        # embedded = self.embedding(temp)
        # print(f'embedded = {embedded}')
        if input_var[0][0] == 50620:
            print(f'input_var====================')
            print(input_var)
        ...
        output_logits = logits.view(batch_size, output_size, -1)

        predicted_softmax = self.function(logits, dim=logits.dim()-1).view(batch_size, output_size, -1)
        return output_logits, predicted_softmax, hidden, attn
    ...
    def forward(self, batch_size, inputs=None, init_state=None,
        ...
        # sanity checkshttps://www.overleaf.com/project/614bdec5645a38d63fcbaf39
        ret_dict = dict()
        ...
        if mode == TEACH_FORCE:
            decoder_output_logits, decoder_output, decoder_hidden, attn = self.forward_step(
            ...
            for di in range(self.max_length):
                decoder_output_logits, decoder_output, decoder_hidden, step_attn = self.forward_step(
                    decoder_input, decoder_hidden, attn_context, latent_variable=latent_variable)
    ...
    def forward4lamol_part1(self, batch_size, inputs=None, init_state=None,
        ...
        if mode == GEN and gen_type == 'beam':
            ...
            if init_state is None:
                ...
                if self.rnn_cell is nn.LSTM:
                    ...
                    decoder_hidden = init_state.repeat(self.n_layers, 1, 1)
        return decoder_input, decoder_hidden, ret_dict

    def forward4lamol_part2(self, decoder_outputs: object, sequence_symbols: object, back_pointers: object, lengths: object, decoder_hidden: object,
                            di: object, decoder_input: object, latent_variable: object, attn_context: object, ret_dict: object, gen_type: object, beam_size: object,
                            batch_size: object,
                            mode: object) -> object:
        # Manual unrolling is used to support random teacher forcing.
        # If teacher_forcing_ratio is True or False instead of a probability,
        ...
        return decoder_output_logits, decoder_outputs, decoder_hidden, ret_dict
