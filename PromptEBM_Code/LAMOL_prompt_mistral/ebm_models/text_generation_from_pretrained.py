...
import json
import random
from LAMOL_prompt_mistral.ebm_models.dgmvae import evaluators, utt_utils
from LAMOL_prompt_mistral.ebm_models.dgmvae.dataset import corpora
from LAMOL_prompt_mistral.ebm_models.dgmvae.dataset import data_loaders
from LAMOL_prompt_mistral.ebm_models.dgmvae.models.sent_models import *
from LAMOL_prompt_mistral.ebm_models.dgmvae.utils import get_time
from LAMOL_prompt_mistral.settings import TOKENIZER, SPECIAL_TOKEN_IDS, FILL_VAL, SAVE_NAME, FINAL_SAVE_NAME, TOKENS_WEIGHT, CONFIG_NAME, parser
from LAMOL_prompt_mistral.ebm_models.dgmvae.multi_bleu import multi_bleu_perl
#from LAMOL.ebm_models.dgmvae.options import get_parser
from LAMOL_prompt.utils import *
import datetime
...
class GMVAE4Lamol(BaseModel):
    ...
    @staticmethod
    def add_args(parser):
        from LAMOL.ebm_models.dgmvae.utils import str2bool
        # Latent variable:
        ...
        parser.add_argument('--max_vocab_cnt', type=int, default=50260)
...
#------------------------------ engine -----------------------------------# 
import numpy as np
from LAMOL.ebm_models.dgmvae.models.model_bases import summary
import torch
from LAMOL.ebm_models.dgmvae.dataset.corpora import PAD, EOS, EOT
from LAMOL.ebm_models.dgmvae.enc2dec.decoders import TEACH_FORCE, GEN, DecoderRNN
from LAMOL.ebm_models.dgmvae.utils import get_dekenize, experiment_name, kl_anneal_function
import os
from collections import defaultdict
import logging
from LAMOL.ebm_models.dgmvae import utt_utils
...
def main():
    ...
    if args.forward_only is False:
        for task_id in range(len(args.tasks)):
            ...
            train(model, train_feed, valid_feed, test_feed, args, evaluator, gen=utt_utils.generate)
    else:
        train_extra_data = []
        train_dataset = [TASK_DICT[t]["train"] for t in tasks]
        train_corpus = getQA_dataset(train_dataset, "train", tasks, train_extra_data)
