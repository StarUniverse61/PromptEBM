...
import quadprog
import io
from ebm_models.dgmvae import evaluators, utt_utils
...
def get_losses(parallel_model, cqa, Y, gen_X, gen_Y, loss_fct):
    if "lll" in args.seq_train_type or "ebm" in args.seq_train_type:
...
def pre_ebm_sample(model, data_feed,  config, epoch, kl_weight=1., gen_size=1283):
    ...
    if hasattr(model, "sampling_for_likelihood") and kl_weight >= config.max_kl_weight:
        ll = utt_utils.calculate_likelihood(model, data_feed, 500, config)  # must
    ...
    if kl_weight >= config.max_kl_weight:
        with open(os.path.join(sampling_file), "w") as f:
            ...
            utt_utils.exact_sampling(model, 46000, args, dest_f=f, sampling_batch_size=gen_size)

    return test_file, sampling_file

def sample_ebm_sequence(ebm_model, data_feed, need_process, qa_results, all_pasts, max_tot_lens, kl_weight=1., num_batch=1, dest_f=None, gen_size=1283):
    from ebm_models.dgmvae import utils
    qa_results = qa_results
    args.batch_size = gen_size
    # test_file, sampling_file = pre_ebm_sample(ebm_model, data_feed=data_feed, kl_weight=kl_weight, config=args, epoch=0)
    ...
    print('gen_size: {}'.format(len(qa_results)))

    #lengths = np.array([args.max_length] * batch_size * beam_size)
    gen_type = 'greedy',
    sample_n = 1,
    batch_cnt = 1,
    return_latent = False,
    vae_kl_weight = 1.
    beam_size = 1
    #ebm_model.forward4lamol(data_feed, vae_kl_weight, gen_type, mode='GEN')
    # print("Saving test to {}".format(test_file))
    ebm_model.eval()
    de_tknize = utils.get_dekenize()
    data_feed.epoch_init(args, shuffle=False, verbose=False)
    ...
    #         break
    '''
     self, batch_size, inputs=None, init_state=None,
     attn_context=None, mode=TEACH_FORCE, gen_type='greedy',
     beam_size=4, latent_variable=None
     dec_outs, posterior_sample_n, sample_z, dec_init_state = ebm_model.forward4lamol_part1(batch_size, data_feed,
    ...
    inputs = np.zeros((len(data_feed.data), max_len), dtype=np.int32)
    labels = np.zeros((len(data_feed.data), 1), dtype=np.int32)
    for idx, row in enumerate(rows):
        inputs[idx, 0:input_lens[idx]] = row.utt
        labels[idx, 0] = row.label
    data_feed_new = inputs
    # ####part1
    decoder_outputs = []  # a list of logprob
    sequence_symbols = []  # a list word ids
    back_pointers = []  # a list of parent beam ID
    lengths = np.array([ebm_model.decoder.max_length] * args.batch_size * beam_size)
    # ####part2
    # decoder_inputs, posterior_sample_n, sample_z, dec_init_state = ebm_model.forward4lamol(data_feed_new,
    #                                                                                  vae_kl_weight=1.,
    #                                                                                  gen_type=gen_type,
    #                                                                                  mode=mode)
    ...
    #     batch = data_feed.next_batch()
        # if batch is None or (num_batch is not None and data_feed.ptr > num_batch): break
    num_mini_batch = 6
    if len(qa_results) != 0:
        batch_ids = random.sample(range(len(data_feed_new)), len(qa_results))
        for i in range(num_mini_batch):
            mini_batch_ids = batch_ids[int((len(qa_results) / num_mini_batch) * i):int((len(qa_results) / num_mini_batch) * (i + 1))]
            # shortest_len = len(qa_results[first_id])
            # decode_batch_size = int(
            #     args.memory_sizes[0] * MEMORY_FACTOR[args.seq_train_type] // (shortest_len + 1) ** LEN_FACTOR)
            # lengths = np.array([args.max_len] * decode_batch_size * beam_size)
            decoder_inputs, posterior_sample_n, sample_z, dec_init_state = ebm_model.forward4lamol(data_feed_new[mini_batch_ids],
                                                                                                   vae_kl_weight=1.,
                                                                                                   gen_type=gen_type,
                                                                                                   mode=mode)
            decoder_input, decoder_hidden, ret_dict = ebm_model.decoder.forward4lamol_part1(len(mini_batch_ids), inputs=decoder_inputs, init_state=dec_init_state, mode=TEACH_FORCE,
                                                                             gen_type=gen_type)
            ...
            # batch_size, inputs = None, init_state = None,
            # attn_context = None, mode = TEACH_FORCE, gen_type = 'greedy',
            beam_size = 4
            # latent_variable = None
            latent_variable = sample_z if ebm_model.concat_decoder_input else None
            batch_size = len(mini_batch_ids)
            # n_inputs = len(input_ids)
            attn_context = None
            # if n_inputs == 0:
            #     break
            ##what is difference between input_ids and decode_input_ids
            # input_ids = torch.stack(input_ids).to(args.device_ids[0]) - 1

            # all_outputs = utt_utils.exact_sampling(model, 46000, MODEL_CONFIG, dest_f=f, sampling_batch_size=100)
            # decoder_outputs, sequence_symbols, back_pointers, lengths, decoder_hidden, di, dec_inputs, latent_variable, attn_context, ret_dict, gen_type, beam_size, batch_size, mode
            all_outputs_logits, all_outputs, dec_last, ret_dict = ebm_model.decoder.forward4lamol_part2(decoder_outputs,
                                                                                   sequence_symbols,
                                                                                   back_pointers,
                                                                                   lengths,
                                                                                   decoder_hidden,
            ...
            outputs = all_outputs_logits
            '''
            not so sure if gpt2 and lstm are quite different for now. Just use the same one for now.
            '''
            # if args.model_name == "gpt2":
            #     pasts = all_outputs_logits[1]
            eos_token = '<|endoftext|>'
            SPECIAL_TOKEN_ID_END = TOKENIZER.convert_tokens_to_ids(eos_token)
            for w_i in range(len(outputs[0, :, 0])):
                next_logits = outputs[..., w_i, :] / args.temperature_qa
                next_tokens = logits_to_tokens(next_logits).cpu()
                for batch_i in range(int((len(qa_results) / num_mini_batch) * i), int((len(qa_results) / num_mini_batch) * (i + 1))):
                    if next_tokens[batch_i - int((len(qa_results) / num_mini_batch) * i)] == 0 or next_tokens[batch_i - int((len(qa_results) / num_mini_batch) * i)] == SPECIAL_TOKEN_ID_END:
                        continue
                    qa_results[batch_i] = torch.cat((qa_results[batch_i], next_tokens[batch_i - int((len(qa_results) / num_mini_batch) * i)]))
            # next_logits = outputs[..., -1, :] / args.temperature_qa
            # next_tokens = softmax_to_tokens(next_softmax).cpu()

        #     for i, cur_id in enumerate(batch_ids):
        #         if next_tokens[i] == SPECIAL_TOKEN_IDS["eos_token"]:
        #             remove_ids.append(cur_id)
        #         else:
        #             ###encounter a small problem. Now, next_tokens = tensor(389)
        #             ###the concat cannot work this way. It is working if we concatenate next_tokens directly
        #             ### next_tokens = tensor([389]). But it seems that this is not correct. Will compare sample_sequence.
...
##part3
'''
###The logic here is that we have a list of starting tokens, then, we add them to input_ids, then model will generate tokens for each line. Once end of tokens reaches for some rows, they are added to
remove_ids. They will be removed from need_process. 
With this procedures, when all rows obtain the end of tokens, the length of need_process will be zeros. Then, sample_sequence is done. 
Note: this is how LAMOL sample sequences. 
'''
def sample_sequence(model, need_process, qa_results, all_pasts, max_tot_lens):
    while len(need_process) > 0:
        ...
        stop = False
        remove_ids = []
        while not stop:         ##input_ids refer to the current tokens as inputs for autoregressive generations
            batch_ids, input_ids, past = [], [], [[] for _ in range(MODEL_CONFIG.num_hidden_layers)]
            while True:
                try:
                    cur_id = next(it)
                    if len(qa_results[cur_id]) > shortest_len:
...
def create_extra_data_ebm(task, test_feed, prev_task, model, ebm_model, train_extra_data):
    if args.real_sample:
        logger.info(f"using real data as extra data")
        return get_real_data(task, train_extra_data)
    task_cnt = args.tasks.index(task)
    model_dir = get_model_dir([prev_task])
    gen_path = os.path.join(model_dir,"lm_test.csv")
    ...
    for task_name in args.tasks[:task_cnt]:
        # qa_results.extend([torch.tensor([SPECIAL_TOKEN_IDS[task_name]]) for _ in range(gen_size)])
        qa_results.extend([torch.tensor([SPECIAL_TOKEN_IDS[task_name]]) for _ in range(gen_size//task_cnt)])
    all_pasts = [[
        torch.empty(2, MODEL_CONFIG.num_attention_heads, 0, MODEL_CONFIG.hidden_size//MODEL_CONFIG.num_attention_heads,
    ...
    ##gen_size refers to batch size, i.e. total number of instances.
    # for i in range(gen_size):
    #     need_process.update([[i, None]])
    #     if len(need_process) > int(args.memory_sizes[0] * 0.12):
    #         sample_ebm_sequence(ebm_model, test_feed, need_process, qa_results, all_pasts, max_tot_lens, gen_size)
    sample_ebm_sequence(ebm_model, test_feed, need_process, qa_results, all_pasts, max_tot_lens, gen_size)
    model.train()

    qa_results = [res.tolist() for res in qa_results]
    train_extra_data.extend(qa_results)
    train_extra_data_out = [TOKENIZER.decode(res) for res in train_extra_data]
    # qa_results = [TOKENIZER.decode(res) for res in qa_results]

    write_extra_data(gen_path, train_extra_data_out)
...
def logits_to_tokens(next_logits):
    filtered_logits = top_k_top_p_filtering(next_logits, top_k=1, top_p=args.top_p_qa)
